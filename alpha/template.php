...
function alpha_css_alter(&$css) {
  ...
  if ($settings['responsive']['enabled']) {
    ...
    $css['ie-normal-grid-defaults']['group'] = CSS_DEFAULT;
    $css['ie-normal-grid-defaults']['weight'] = 50;
    $css['ie-normal-grid-defaults']['basename'] = 'ie-normal-grid-defaults';
    $css['ie-normal-grid-defaults']['browser'] = array('IE' => '(lt IE 9)&(!IEMobile)', '!IE' => FALSE);
    ...
    foreach($grid['columns'] as $columns => $path) {
      ...
      if (isset($css[$path])) {
        ...
        $css['ie-normal-grid-defaults-' . $columns]['group'] = CSS_DEFAULT;
        $css['ie-normal-grid-defaults-' . $columns]['weight'] = 50;
        $css['ie-normal-grid-defaults-' . $columns]['basename'] = 'ie-normal-grid-defaults-' . $columns;
        $css['ie-normal-grid-defaults-' . $columns]['browser'] = array('IE' => '(lt IE 9)&(!IEMobile)', '!IE' => FALSE);
      }
    }
  }
}
