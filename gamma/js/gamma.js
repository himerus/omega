...
{
  obj.onload = null;
  var newthis = gamma(obj);
  if (gamma.browser.msie)
  {
...
gamma(function(){
  gamma('.service-links img').data('desaturate', {'level':0.5});

  gamma(".switched_images").bind("mouseenter mouseleave", function(event) {
      if (event.type == 'mouseenter')
      {
        //gamma(".des:not(.color)", this).fadeOut(1000);
        gamma(".service-links img.color", this).fadeIn(500);
      }
      if (event.type == 'mouseleave')
      {
        //gamma(".des:not(.color)", this).fadeIn(1000);
        gamma(".service-links img.color", this).fadeOut(500);
      }
    });
});
*/